...
export const articlesRouter = router({
  ...
  changeStatus: protectedProcedure
    .input(
      z.object({
        id: z.string(),
        status: z.enum(["draft", "published", "archived", "review", "approved", "scheduled", "killed"]),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const updatedArticle = await db
        .update(articles)
        .set({ status: status, publishedAt: status === 'published' ? new Date() : currentArticle[0].publishedAt })
        .where(eq(articles.id, id))
        .returning();

      // تسجيل التغيير في سجل سير العمل
      await db.insert(workflowHistory).values({
        id: nanoid(),
        articleId: id,
        fromStatus: currentArticle[0].status,
        toStatus: status,
        userId: ctx.user.id,
        comment: comment,
      });
  ...
  listEditorialComments: protectedProcedure
  ...
  addEditorialComment: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin' && ctx.user.role !== 'editor' && ctx.user.role !== 'writer') {
        throw new Error('غير مصرح لك بإضافة تعليقات تحريرية');
      }
  ...
  resolveEditorialComment: protectedProcedure
    .input(z.object({ id: z.string() }))
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin' && ctx.user.role !== 'editor') {
        throw new Error('غير مصرح لك بحل التعليقات التحريرية');
      }
      ...
      const updatedComment = await db
        ...
        .where(eq(editorialComments.id, input.id))
        .returning();
  ...
  generateEditorialElements: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin' && ctx.user.role !== 'editor') {
        throw new Error('غير مصرح لك باستخدام هذه الميزة');
      }

      // ... (الكود الخاص بتوليد العناصر التحريرية)
